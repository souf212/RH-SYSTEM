...
{ 
    public class UtilisateurController : Controller
    {
        ...
        {
            var model = new ManagerCreationVM
            {
                AvailableEmployees = await _userService.GetAvailableEmployeesAsync()
            };
            return View(model);
        }
        ...
        [Authorize(Roles = "Admin")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateManager(ManagerCreationVM managerVM)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    managerVM.Role = "Manager";

                    // Create the manager with selected team members
                    await _userService.CreateUserAsync(managerVM, managerVM.TeamMemberIds);

                    TempData["Success"] = "Manager créé avec succès";
                    return RedirectToAction(nameof(Index));
                }

                // If we got this far, something failed, reload the available employees
                managerVM.AvailableEmployees = await _userService.GetAvailableEmployeesAsync();
