...
def get_employee_mapping():
    try:
        conn = mysql.connector.connect(
            host="localhost",
            user="root",
            password="Mohamed*",  # 🔴 Update this with your MySQL password
            database="HR_Management_System1"
        )
        cursor = conn.cursor()

        # ✅ Fetch `IdEmploye`, `IdUtilisateur`, and `Nom`
        cursor.execute("SELECT IdEmploye, IdUtilisateur, LOWER(Nom) FROM Employes")
